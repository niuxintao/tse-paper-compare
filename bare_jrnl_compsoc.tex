...
\end{table*}
 ...
 2)recall :  our algorithms perform the best in all the cases. which means our algorithms can find more MFSs than others in a failing configuration. This owe to the way our algorithm adopt, that consider as more scenario as possible. Especially consider the multiple , overlapped and the introduce. Another note is that fir exception 1 and exception 3 and 4, the LG algorithm get the recall 0, it is a signal that in this three cases, the MFSs in the SUT have degrees larger than 2.

 3)precise : our algorithms also performs the best in most of the cases. except for the exception 2 are worse than FIC and RI and OFOT and LG. As we all know, only one thing can influence the precise, which is the introduce rate. As we have consider it, it will not affect our result.

 Besides, we can find a little different, that is the recall and precise is best in 4, second best in 2 , little worse in 1, the worst in 3. This is coincided with the intro rate, which means that the intro rate is defined well and the intro rate really indicate the performance in algorithms.
...
Delta debugging [5] proposed by Zeller is an adaptive divide 朼nd-conquer approach to locating interaction fault. It is very efficient and has been applied to real software environment. Zhang et al. [4] also proposed a similar approach that can identify the failure-inducing combinations that has no overlapped part efficiently,
