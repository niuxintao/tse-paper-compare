...
The subject systems for these studies are HSQLDB(2.0rc8). HSQLDB is a database management system written in pure java.　All of these software share the same testing proceedings as depicted in Fig.\ref{proceeding}.
\begin{figure}
 \centering
...
%execute
We sign this input model as ($2^13, 3^3$). There are $ 2^13 * 3 ^ 3 = 221184 $ possible configurations in total. We executed the test script under each of the 221184 configurations and record their result. In specific, there are 147472 configurations triggered exceptions, and others passed this test. These exceptions can classify 4 types according to the exception traces. Through viewing the exception trace info and the source code of test script, we sorted out the priorities of these exceptions. Table \ref{faultyPriority} shows the detail the 4 types of exceptions. The "exception ID" column shows the id of the exception. The "lower priority "column shows the IDs of exception which have lower priority than this exception. The "num of configs" show the number of test configurations under which the test script triggered this type of exception when executing.
