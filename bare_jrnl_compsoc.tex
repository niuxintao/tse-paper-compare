...
\end{table*}
 ...
 \subsubsection{Study setup}
 To utilize the result of each algorithm, we build a voting system. Thus, choose some algorithms and take a voting schedule: if the a schema is identified more than two algorithms, then we take it as MFS, otherwise, we will discard it. We will vary the number of algorithms and adopt different combination of algorithms to observe the result. In specific we will take 2, 3, 4, 5 and 6 algorithms as the voters respectively. In addition, we will take all the combination of algorithms for a specific number. Take 2 for example we will choose ChainFeedBack and OFOT as a combination, ChainFeedBack and RI as another combination and so on. The total number combination of algorithms for a specific number $n$ is $\binom{6}{n}$.

 For each voting combination, we will also record the metric "recall" and "precise". We just want to know, whether there exist some voting combination can have a advantage over all the algorithms consists it. And if so, how much promotion is the voting combination got compared to the best of the combination algorithms. The result is show the percentage of in two facts: if it has a promotion over all the algorithms,  .
 \subsubsection{result and analysis}
 \begin{table*}\renewcommand{\arraystretch}{1.3}
