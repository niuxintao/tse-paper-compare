...
As MFSs in the softwares are figured out, we can easily count the number of configurations contain multiples MFSs and MFSs that overlapped each other, the number of  MFSs that have a degree larger than 2.  But for the possibility of introducing newly MFSs when generating extra configuration (brief as possibility of introducing), we yet can't measure it for we didn't define how to compute the "possibility". We will give a formula to compute the possibility of introducing later, before then we will explain how is this formula derived.
...
For a test case of which we want to identify the MFSs, say test case A,  if we change one factor of it to generate a new test case, say test Case B.  Then assume in this step we introduced a new failure-inducing schema, but meanwhile we didn't break the original failure-inducing schema in the test case A when we generate B.
...
Still for that case, 
So we just should consider when the introduce newly faulty schemas may affect our identifying result.
That is, when we break the original MFS, and introduce a new MFS.
So then the introduce rate is easily defined:
We just need to compute the change distance between the failure-inducing schemas in this software.
Change distance of schema:
A  B
T(A,B) -- the parameters in A but not in B
S(A,B) -- the parameters in both A and B, but their value is different.
$P_{i}$ -- the number of values in i th parameter of SUT

$A_{i}$ -- the value of the i th paramter of SUT in Schema A.

ChangeDistance(A, B) = $|T(A,B)| +\sum_{i \in T(B,A)}\left(P_{i} / 2\right) + \sum_{i \in S(A,B)}\left( |A_{i} - B_{i}| / 2\right)$

Then the introduce rate of a SUT is defined as
MFSs -- all the MFS in the SUT

$\dfrac {\sum_{a , b \in MFSs, a \neq b }\left(ChangeDistance(a,b)\right)} {|MFSs|\times |MFSs - 1|}$
