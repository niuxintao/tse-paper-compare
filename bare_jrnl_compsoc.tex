...
\section{Evaluation with simulated SUT}
...
Some parameters of these algorithm are assigned: for SP, the degree of MFSs we set is 4, for CTA,the confidence factor of classified tree algorithm is 0.25 as same as [].
...
We will discuss the results into five group according to the experiment set up.
...
There are some intuitive information we can learn from this figure. First, the configurations needed of some algorithms didn't change along with the change of SUT. These algorithms are AIFL, CTA, OFOT,LG, while others need different configurations according to the SUT they are facing. Second, some algorithms shows a obvious disparity in this figure, we can easily find that the configurations that needed to shows obvious rank: . 

For the remaining algorithms, their results are tangled with each other. So to rank them, we give an average configurations each algorithms needed as follows:
Chain 9.25, FeedBack 11.25, fic 7.96, ri 8 ofot 16 lg 11 , sp .29, aifl 36, rtr 10.75 TRT-NA 23.75   CTA 16

Overall, we can give an initial rank in the number of configurations each algorithm needed as:  FIC-BS<RI<LG<TRT<CHAIN<OFOT = CTA <TRT-NA<AIFL<SP.
...
2)Figure \ref{fig_double} list the result of experiment 2. This figure doesn't show the number of configurations of each algorithms , it just show the average precise and recall for each algorithms. We think it is meaningless. We will attach the average of configurations each algorithms needed in the following which can both get 1 algorithms.

This figure give us a clear result of . We can find the following algorithms: chain, lg, sp, aifl, trt and trt-na can get the precise and recall 1, which means that they all can deal with the multiple results.  and the algorithms ofot and cta can't handle this. As for the remaining algorithms ri and fic, they have a precise 1 , but didn't have recall 1, this is becasue they can't handle the condition if two MFSs have overlapped part, in such condition, they can only identify one.

We just only gives the algorithms can both get 1 the average configuratios as follows:
...
3)The third experiment's result is shown in fig \ref{fig_import}.
