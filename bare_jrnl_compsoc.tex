...
3)The third experiment aims at learning the ability that each algorithm handling the introducing newly MFSs. To accomplish this goal, We firstly inject two MFSs in a SUT, and then feed each algorithm a failing configuration which contain only one MFS,  remaining another MFS not in it. To increase the possibility of introducing newly MFSs for each algorithm, we set the second MFS with degree 1, which may be introduced by may just changing one factor of the original failing configuration.  The first MFS in the original configuration has degree 2. So there are  $\binom{8}{2} \times \binom{8}{1} = 224$ possible SUTs in this experiment. The same as experiment 2, we will record the "recall" and "precise" of each algorithm.

and double MFSs, these MFS area each possible, give a test configuration with this two MFSs see the precise and recall 3) For a SUT with 8 parameters, and double MFSss, give a failing test configuration just contain one MFS, , we do so that give an chance that the extra test configuration can introduce another faulty schema. 4) for a SUT with single MFS, degree 2, and vary the number of parameters, are 8 , 9 , 10,12,15,20,30,40,60,80,100,120. average number of additional configurations 5)for a SUT with 8 parameters, a single MFS, the degree vary from 1, 2,3,4,5,6,7,8. and average number of additional test configurations.

4)For the fourth experiment is a l 
