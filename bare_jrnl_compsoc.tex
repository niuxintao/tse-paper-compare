...
The subject systems for these studies are HSQLDB(2.0rc8). HSQLDB is a database management system written in pure java.

\subsection{experimental setup}
Before we use these programs for study, we should obtain some basic information of them. For each subject program we will do the following process, 1)Build the input configuration model of the subject program 2)execute the test case under each possible configuration and record their executed result. 3)statistic all the types of fault during testing and judge their priority.

We will depict these process and result for each subject in detail.
\subsubsection{HSQLDB}
Through the developers' community of HSQLDB in sourceforge, we find a buggy version and its faulty description in [], and a test script which can reproduce the bug is attached. The original test script only considered one option which have two values,remain the other options setting default configuration. We add some other options of the database management software, combined with the option in test script we derive the input configuration model in listed table \ref{modelHSQLDB}.
...
\begin{table}\renewcommand{\arraystretch}{1.3}
  ...
  \begin{tabular}{c*{2}{p{0.53\columnwidth}}}
  \hline
  \bfseries Database properties &   \bfseries values \\
   \hline
...
Before we describe our case studies we should first describe the subject programs in our case study.
Database systems is customable and complicated. so we choose a database as our system. Through viewing its developer's community . we find a bug description which have been repaired[]. We take this version and its test script. To be more adaptive for our testing approach, we add some configuration options to the test script. To be more specific, we can see the followed model  the test model is as follows,

from which the configuration file which controlled the behavior of data base is the input model which listed as follows: we can simplify as this
$ 2^13 * 3 ^ 3 = 221184 $

the source code of HSQLDB is just a jar  library, the test file is a common script file that drive one running case of the sqldb.

Our testing paradigm is writing the configure file and executing the test script.

\subsubsection{preprocessing}
To evaluate the performance of different algorithms , we should know the real MFS in this software, the original webpage is a hint, but is not enough, as it is just adapt two options. And now we add more options , which make the problem more complicated. So we take the followed steps:
1.execute the test script under each possible configuration.  we running  Particularly, we should run $ 2^13 * 3 ^ 3 = 221184 $ test cases. and This process is time consuming.
2.collect the failure information and their priority. this is done by We also by static review of the test script code.
3.Use definition to exhaustive get the real faulty schema information.
\subsubsection{result}
The result is as follows: four types of fault. their priority is as:
we get the followed faulty schema for each failure.

\subsection{Study 1: what the schemas like in test configurations of HSQLDB}
In this study, we want to answer the first question. Do they have multiple faulty schemas, and do they have higher degrees.And can they import bugs.
...
Nie抯 approach in [3] and [6] first separates the faulty-possible tuples and healthy-possible tuples into two sets. Subsequently, by changing a parameter value at a time of the original test configuration, this approach generates extra test configurations. After executing the configurations, the approach converges by reducing the number of tuples in the faulty-possible sets.
...
C. Mart韓ez [8-9] proposed two adaptive algorithms. The first one needs safe value as their assumption and the second one remove the assumption when the number of values of each parameter is equal to 2. Their algorithms focus on identifying the faulty tuples that have no more than 2 parameters.
