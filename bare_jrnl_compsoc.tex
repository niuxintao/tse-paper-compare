...
\end{table*}
 ...
 2)recall :  our algorithms perform the best in all the circumstances, which means our algorithms can find more MFSs than others algorithms when identifying the MFSs in a failing configuration. The reason why our approach can be the best at this metric is that we consider more scenarios when identifying the MFSs than other approaches, especially when facing multiple , overlapped MFSs in a test configuration, high degree MFSs. (((and the condition when extra test configurations introduce newly MFSs.))) Other approach may consider some of them, but none of them consider all these scenarios. Another point that needed note is that for exception 1, 3 and 4, the recall of LG algorithm is 0, it is a signal that in this three circumstances, the MFSs in the SUT have degrees larger than 2.

 3)precise : our algorithms also performs the best in most of the cases, except for the exception 2 in which case our algorithm is worse than FIC ,RI ,OFOT and LG.  As we have discussed before, this metric is influenced by the introduce rate. So the result is a good evidence that the feedback action in our approach is effective at avoiding the effects along with the introducing newly MFSs. Algouth it can't get the best in all the cirumstances, we yet have confidence it can give a.
 ...
 \subsubsection{Study setup}
 In this study, we just give pair wise combination of algorithms result.
 \subsubsection{result and analysis}
